...
class MHA():
    ...
    def att_score(self,query, key, value, decoder = False):
        d_k = query.shape[-1]
        ...
        if decoder:
            attention_scores = torch.broadcast_to(MHA(self.d_k,self.h,self.seq_len)._generate_square_subsequent_mask(self.seq_len),attention_scores.shape) + attention_scores
    ...
    def calculate(self,emb_mat,decoder = False,enc_output = False):
        ...
        if decoder:
            att_score_mat, self.attention_scores = MHA(self.d_k,self.h,self.seq_len).att_score(query, key, value,decoder=decoder)
        else:
            att_score_mat, self.attention_scores = MHA(self.d_k,self.h,self.seq_len).att_score(query, key, value)
