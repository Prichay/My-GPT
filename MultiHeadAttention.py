...
class MHA(torch.nn.Module):
    ...
    def att_score(self,query, key, value,seq_len,pad_mask = False,dec_pad_mask = False,cross_att = False):
        d_k = query.shape[-1]
        ...
        if pad_mask:
            ...
            look_ahead = torch.broadcast_to(self._generate_square_subsequent_mask(seq_len),(query @ key.transpose(-2, -1)).shape)
            ...
            look_ahead_query = torch.broadcast_to(self._generate_square_subsequent_mask(seq_len),(query @ key.transpose(-2, -1)).shape)
    ...
    def calculate(self,emb_mat,enc_output = False,pad_mask = False,dec_pad_mask=False, cross_att = False):
        ...
        if dec_pad_mask:
            att_score_mat, self.attention_scores = self.att_score(query, key, value,seq_len,dec_pad_mask=dec_pad_mask)
        elif pad_mask:
            att_score_mat, self.attention_scores = self.att_score(query, key, value,seq_len,pad_mask = pad_mask)
        elif cross_att:
            att_score_mat, self.attention_scores = self.att_score(query, key, value,seq_len,cross_att=cross_att)
